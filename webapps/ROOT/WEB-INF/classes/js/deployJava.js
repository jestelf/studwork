var deployJava = {
    ...
    writeAppletTag:function(attributes, parameters) {
        var s = deployJava.getAppletTag(attributes, parameters);
        document.write(s);
    ...
    getBrowser:function() {
        ...
        if ((navigator.vendor) && (navigator.vendor.toLowerCase().indexOf('apple') != -1) && (browser.indexOf('safari') != -1)) {
            ...
            if (deployJava.debug) {
                alert('We claim to have detected a Chrome browser.');
            }
            return'Chrome';
            ...
            if (deployJava.debug) {
                alert('We claim to have detected "Opera".');
            }
            return'Opera';
        } else {
            if (deployJava.debug) {
    ...
    isJavaInstalled:function(version) {
        var jres = deployJava.getJREs();
        for (var j = 0; j < jres.length; j++) {
            if (deployJava.compareVersions(jres[j], version)) {
                return true;
            }
        }
        return false;
